...
Public Class Form1
    ...
    Private Sub CheckGameOver()
        For Me.x = 1 To NumOfInvaders
            If Invaders(x).Top + Invaders(x).Height >= shooter.Top And Invaders(x).Visible = True Then
                TimerMain.Enabled = False
                Me.x = NumOfInvaders
                MsgBox("Game over! Skor kamu : " & Val(lbscorecount.Text * 1) & ". Click OK dan next.")
                resetscore()
                'PlayAgain()
        ...
        If ShotDown = NumOfInvaders Then
            ...
            'PlayAgain()
            koneksi()
            MsgBox("Kamu menang! Skor kamu : " & Val(lbscorecount.Text * 1) & ". Click OK dan next.")
            'mySqlCommand = New MySqlCommand("update score set skor = '" & lbscorecount.Text & "' where nama = '" & Label2.Text & "'", mySqlConn)
    ...
    '        Timer1.Enabled = False
    '        Timer2.Enabled = False
    '        MsgBox("Kamu kehabisan waktu. Skor kamu : " & Val(lbscorecount.Text * 1) & ". Click OK untuk mulai ulang.")
    '        'PlayAgain()
    '        'resetscore()
    '    End If
    'End Sub
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        quiz_score.TextBox2.Text = Val(lbscorecount.Text) * 1
        Me.Hide()
        quiz_score.Show()
    End Sub
End Class
